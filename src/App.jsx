...
/* ─── USERS ─── */
const USERS = [
  { id:"seba", name:"Seba", avatar:"S", color:"#6366f1", colorLight:"#eef2ff" },
  { id:"dani", name:"Dani", avatar:"D", color:"#f59e0b", colorLight:"#fffbeb" },
  { id:"jime", name:"Jime", avatar:"J", color:"#10b981", colorLight:"#ecfdf5" },
];
...
const AREA_PALETTE = ["indigo","amber","green","red","sky","purple","rose","teal","orange","gray"];
const TYPE_PALETTE = { analisis:"sky", dolor:"red", pendiente:"amber" };
...
function areaPalette(area) {
  const idx = [...(area||"")].reduce((a,c)=>a+c.charCodeAt(0),0);
  return PALETTE[AREA_PALETTE[idx % AREA_PALETTE.length]];
}
function typePalette(type) { return PALETTE[TYPE_PALETTE[type]||"gray"]; }
...
async function fbSet(uid,col,id,data) { try { await setDoc(uDoc(uid,col,id),data); } catch(e){console.error(e);} }
async function fbDel(uid,col,id)      { try { await deleteDoc(uDoc(uid,col,id)); } catch(e){console.error(e);} }
async function fbSetCfg(uid,data)     { try { await setDoc(uCfg(uid),data); } catch(e){console.error(e);} }
...
function Badge({ label, palette, icon, size="md" }) {
  ...
  return (
    <span style={{display:"inline-flex",alignItems:"center",gap:4,background:p.bg,color:p.fg,border:`1px solid ${p.border}`,fontSize:11,fontWeight:600,padding:pad,borderRadius:20,whiteSpace:"nowrap"}}>
...
function TypeBadge({ type, size="md" }) {
  ...
  return <Badge label={ITEM_TYPES[type]||type} palette={p} icon={TYPE_ICON[type]||"ti-tag"} size={size}/>;
}
function AreaChip({ area, size="md" }) {
  return <Badge label={area} palette={areaPalette(area)} size={size}/>;
}

function StatCard({ label, value, icon, palette, onClick }) {
  const p = palette || PALETTE.gray;
  return (
    <div onClick={onClick} style={{background:p.bg,border:`1px solid ${p.border}`,borderRadius:12,padding:"16px 18px",cursor:onClick?"pointer":"default"}}>
  ...
  return (
    <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
      {photos.map(p=>(
        <div key={p.id} style={{position:"relative",width:sz,height:sz,flexShrink:0}}>
          ...
          <button onClick={()=>onChange(photos.filter(x=>x.id!==p.id))}
            style={{position:"absolute",top:-5,right:-5,width:18,height:18,borderRadius:"50%",background:"#111827",color:"#fff",border:"none",cursor:"pointer",fontSize:9,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
        </div>
      ))}
      <label style={{width:sz,height:sz,border:"1.5px dashed #d1d5db",borderRadius:8,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"#9ca3af",gap:2,flexShrink:0,background:"#f9fafb"}}>
        <i className="ti ti-camera-plus" style={{fontSize:compact?16:20}} aria-hidden="true"/>
...
function BulletNotes({ bullets, onChange, placeholder="Agregar punto…" }) {
  ...
  function addAfter(idx){const n=[...bullets];n.splice(idx+1,0,"");onChange(n);setTimeout(()=>refs.current[idx+1]?.focus(),30);}
  function removeAt(idx){
    if(bullets.length===1){onChange([""]);return;}
    const n=[...bullets];n.splice(idx,1);onChange(n);setTimeout(()=>refs.current[Math.max(0,idx-1)]?.focus(),30);
  }
  ...
  return (
    <div style={{background:"#f9fafb",borderRadius:8,border:"1px solid #e5e7eb",padding:"10px 12px"}}>
      {bullets.map((b,idx)=>(
        <div key={idx} style={{display:"flex",alignItems:"center",gap:8,marginBottom:idx<bullets.length-1?6:0}}>
          <span style={{color:"#6366f1",fontSize:16,userSelect:"none",flexShrink:0,lineHeight:1}}>•</span>
          ...
          {bullets.length>1&&<button onClick={()=>removeAt(idx)}
            style={{background:"none",border:"none",cursor:"pointer",color:"#9ca3af",fontSize:12,padding:"0 2px",flexShrink:0}}>✕</button>}
        </div>
      ))}
      <button onClick={()=>addAfter(bullets.length-1)}
        style={{marginTop:8,fontSize:11,color:"#6366f1",background:"none",border:"none",cursor:"pointer",padding:"2px 0",display:"flex",alignItems:"center",gap:4,fontWeight:500}}>
        <i className="ti ti-plus" style={{fontSize:11}} aria-hidden="true"/>Agregar punto
      </button>
    </div>
  );
}
function BulletDisplay({ bullets }) {
  ...
  return <ul style={{margin:"6px 0 0",padding:"0 0 0 14px",listStyleType:"none"}}>
    {items.map((b,i)=>(
      <li key={i} style={{fontSize:13,color:"#4b5563",lineHeight:1.6,display:"flex",gap:8,alignItems:"baseline"}}>
        <span style={{color:"#6366f1",fontSize:14,lineHeight:1,flexShrink:0}}>•</span>{b}
      </li>
    ))}
  </ul>;
}
  ...
  return (
    <div style={{borderTop:"1px solid #f3f4f6",paddingTop:12,marginTop:12}}>
      ...
      {item.status==="abierto"&&onResolve&&(
        <button onClick={()=>onResolve(item.id)} style={{marginTop:10,width:"100%",padding:"8px",fontSize:12,fontWeight:600,background:"#f0fdf4",color:"#166534",border:"1px solid #86efac",borderRadius:8,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
          <i className="ti ti-check" style={{fontSize:13}} aria-hidden="true"/>Marcar como resuelto
        </button>
      )}
    </div>
  );
...
function ItemCard({ item, locationName, visitDate, onResolve, onAddComment, onEditItem, showLocation=true }) {
  ...
  const comments=item.comments||[];
  const p=typePalette(item.type);
  function saveEdit(){onEditItem(item.id,editDesc);setEditing(false);}
  return (
    <div style={{background:"#fff",border:`1px solid ${item.status==="resuelto"?"#f3f4f6":p.border}`,borderRadius:12,padding:"14px 16px",marginBottom:8,opacity:item.status==="resuelto"?0.65:1,borderLeft:`3px solid ${item.status==="resuelto"?"#e5e7eb":p.fg}`}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,marginBottom:8}}>
        ...
        <span style={{fontSize:11,color:"#9ca3af",whiteSpace:"nowrap",flexShrink:0,marginTop:2}}>{fmtS(visitDate)}</span>
      </div>
      {editing?(
        ...
        <div style={{display:"flex",alignItems:"flex-start",gap:6,marginBottom:6}}>
          ...
          {item.status==="abierto"&&onEditItem&&(
            <button onClick={()=>{setEditDesc(item.description);setEditing(true);}}
              style={{background:"none",border:"none",cursor:"pointer",color:"#9ca3af",padding:"2px",flexShrink:0,marginTop:1}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:10}}>
        <div style={{display:"flex",gap:12,alignItems:"center",flexWrap:"wrap"}}>
          {showLocation&&locationName&&<span style={{fontSize:12,color:"#6b7280",display:"flex",alignItems:"center",gap:4}}>
            <i className="ti ti-building-store" style={{fontSize:12}} aria-hidden="true"/>{locationName}
          </span>}
          <span style={{fontSize:12,color:"#9ca3af",display:"flex",alignItems:"center",gap:4}}>
            <i className="ti ti-calendar" style={{fontSize:12}} aria-hidden="true"/>{fmt(visitDate)}
          </span>
        </div>
        <button onClick={()=>setExpanded(e=>!e)} style={{display:"flex",alignItems:"center",gap:4,fontSize:12,background:"none",border:"none",cursor:"pointer",color:"#6366f1",padding:0,fontWeight:500}}>
          ...
          <i className={`ti ${expanded?"ti-chevron-up":"ti-chevron-down"}`} style={{fontSize:12}} aria-hidden="true"/>
        </button>
      </div>
...
function NavBtn({ icon, label, active, badge, onClick, sub, color }) {
  return (
    <button onClick={onClick} style={{display:"flex",alignItems:"center",gap:9,width:"100%",padding:sub?"7px 16px 7px 34px":"9px 16px",background:active?"#eef2ff":"transparent",border:"none",borderLeft:active?`3px solid ${color||"#6366f1"}`:"3px solid transparent",cursor:"pointer",textAlign:"left",color:active?"#4338ca":"#6b7280",fontSize:sub?13:14,fontWeight:active?600:400}}>
      <i className={`ti ${icon}`} style={{fontSize:sub?14:16,color:active?(color||"#6366f1"):"inherit"}} aria-hidden="true"/>
      ...
      <div>
        ...
        {typeAdding?(
          <div style={{display:"flex",gap:4}}>
            ...
            <button onClick={()=>{const v=confirmNewType();if(v)setType(v);}} style={{padding:"7px 10px",background:"#f0fdf4",color:"#166534",border:"1px solid #86efac",borderRadius:8,cursor:"pointer",fontSize:13}}>✓</button>
            <button onClick={()=>{setTypeAdding(false);setTypeNewVal("");}} style={{padding:"7px 10px",background:"transparent",border:"1px solid #e5e7eb",color:"#6b7280",borderRadius:8,cursor:"pointer",fontSize:12}}>✕</button>
          </div>
        ):(
          <select value={type} onChange={e=>{if(e.target.value==="__new__")setTypeAdding(true);else setType(e.target.value);}} style={inp}>
      ...
      <div>
        ...
        {areaAdding?(
          <div style={{display:"flex",gap:4}}>
            ...
            <button onClick={()=>{const v=confirmNewArea();if(v)setArea(v);}} style={{padding:"7px 10px",background:"#f0fdf4",color:"#166534",border:"1px solid #86efac",borderRadius:8,cursor:"pointer",fontSize:13}}>✓</button>
            <button onClick={()=>{setAreaAdding(false);setAreaNewVal("");}} style={{padding:"7px 10px",background:"transparent",border:"1px solid #e5e7eb",color:"#6b7280",borderRadius:8,cursor:"pointer",fontSize:12}}>✕</button>
          </div>
        ):(
          <select value={area} onChange={e=>{if(e.target.value==="__new__")setAreaAdding(true);else setArea(e.target.value);}} style={inp}>
...
function InlineAddItem({ allFormAreas, allTypeOptions, onAdd, onCancel, showCancel=false, ...shared }) {
  ...
  return (
    <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,padding:"16px",boxShadow:"0 1px 3px rgba(0,0,0,.06)"}}>
      ...
      <div style={{marginBottom:10}}>
        <input value={desc} onChange={e=>setDesc(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleAdd()}
          placeholder="Descripción del ítem…" style={inp}/>
      </div>
...
const baseInp={padding:"8px 12px",borderRadius:8,border:"1px solid #e5e7eb",background:"#f9fafb",color:"#111827",fontSize:14,width:"100%",boxSizing:"border-box",fontFamily:"inherit"};
const smlInp={...baseInp,fontSize:13,padding:"7px 11px"};

function LoginScreen({ onSelect }) {
  return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#eef2ff 0%,#f0fdf4 100%)",fontFamily:"system-ui,sans-serif",padding:24}}>
      <div style={{marginBottom:40,textAlign:"center"}}>
        <div style={{width:56,height:56,borderRadius:14,background:"#6366f1",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px",boxShadow:"0 4px 14px rgba(99,102,241,.35)"}}>
          <i className="ti ti-building-store" style={{fontSize:26,color:"#fff"}} aria-hidden="true"/>
        </div>
      ...
      <div style={{display:"flex",gap:16,flexWrap:"wrap",justifyContent:"center"}}>
        {USERS.map(u=>(
          <button key={u.id} onClick={()=>onSelect(u)}
            ...
            <div style={{width:60,height:60,borderRadius:"50%",background:u.color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,fontWeight:700,color:"#fff",boxShadow:`0 4px 12px ${u.color}66`}}>
              {u.avatar}
            </div>
...
export default function App() {
  ...
  function selectUser(u) {
    ...
    unsubRef.current=[
      onSnapshot(uCol(uid,"locations"),snap=>{setLocations(snap.docs.map(d=>d.data()));if(!fired.locs){fired.locs=true;check();}}),
      onSnapshot(uCol(uid,"visits"),snap=>{setVisits(snap.docs.map(d=>d.data()));if(!fired.visits){fired.visits=true;check();}}),
      onSnapshot(uCol(uid,"items"),snap=>{setItems(snap.docs.map(d=>d.data()));if(!fired.items){fired.items=true;check();}}),
      onSnapshot(uCfg(uid),snap=>{if(snap.exists()){const d=snap.data();setCustomAreas(d.customAreas||[]);setCustomTypes(d.customTypes||[]);}if(!fired.cfg){fired.cfg=true;check();}}),
    ];
    setView("dashboard");
  }
  function logout(){
  ...
  function addComment(itemId,text){
    ...
    const updated={...item,comments:[...(item.comments||[]),c]};
  ...
  function addEditItem(item){
    const ni={...item,id:genId(),visitId:editVid,locationId:visits.find(v=>v.id===editVid)?.locationId,createdAt:editDate,status:"abierto",comments:[]};
    setEditItems(p=>[...p,ni]);setShowAddInEdit(false);
  }
  async function saveEdit(){
    const orig=visits.find(v=>v.id===editVid);
    const updVisit={...orig,date:editDate,bullets:editBullets,photos:editPhotos};
  ...
  if(!loaded)return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#f0f9ff"}}>
      <div style={{textAlign:"center",color:"#6b7280"}}>
        <div style={{width:36,height:36,border:"3px solid #e5e7eb",borderTopColor:"#6366f1",borderRadius:"50%",margin:"0 auto 12px",animation:"spin .8s linear infinite"}}/>
        Cargando…<style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
      </div>
    </div>
  );
    ...
    <div style={{display:"flex",minHeight:"100vh",fontFamily:"system-ui,sans-serif",background:"#f8f9fb"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div>
              ...
              <button onClick={logout} style={{fontSize:11,color:"#9ca3af",background:"none",border:"none",cursor:"pointer",padding:0,display:"flex",alignItems:"center",gap:3,marginTop:1}}>
                <i className="ti ti-logout" style={{fontSize:11}} aria-hidden="true"/>Cambiar usuario
              </button>
            </div>
          </div>
        </div>
      ...
      <main style={{flex:1,overflow:"auto"}}>
        ...
        {view==="dashboard"&&(
          <div style={{padding:"28px",maxWidth:780}}>
            <div style={{marginBottom:24}}>
              <h1 style={{fontSize:24,fontWeight:700,margin:"0 0 4px",color:"#111827"}}>Inicio</h1>
              <div style={{fontSize:14,color:"#6b7280"}}>Bienvenido, {currentUser.name}</div>
            </div>
            ...
            {allAreas.length>0&&(
              <div style={{marginBottom:28}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(170px,1fr))",gap:8}}>
                  {allAreas.map(area=>{const n=openItems.filter(i=>i.area===area).length;const p=areaPalette(area);return(
            ...
            {allResolvedAreas.length>0&&(
              <div style={{marginBottom:28}}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                  {allResolvedAreas.map(area=>{const n=resolvedItems.filter(i=>i.area===area).length;const p=areaPalette(area);return(
            ...
            {locations.length===0&&(
              <div style={{textAlign:"center",padding:"60px 24px",background:"#f0f9ff",borderRadius:16,border:"1px dashed #7dd3fc"}}>
                <i className="ti ti-building-store" style={{fontSize:40,display:"block",marginBottom:12,color:"#7dd3fc"}} aria-hidden="true"/>
          ...
          <div style={{padding:"28px",maxWidth:740}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:22}}>
              ...
              <button onClick={()=>setShowAddLoc(true)} style={{display:"flex",alignItems:"center",gap:6,padding:"8px 16px",background:"#6366f1",color:"#fff",border:"none",borderRadius:8,cursor:"pointer",fontSize:13,fontWeight:600}}>
                <i className="ti ti-plus" style={{fontSize:14}} aria-hidden="true"/> Agregar local
              </button>
            </div>
            {showAddLoc&&(
              <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,padding:"16px",marginBottom:16,boxShadow:"0 2px 8px rgba(0,0,0,.06)"}}>
            ...
            {locations.map(loc=>{
              ...
              return(
                <div key={loc.id} style={{background:"#fff",border:"1px solid #f3f4f6",borderRadius:12,padding:"14px 16px",marginBottom:8,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
                  {isEditing?(
                    <div style={{display:"flex",gap:8,alignItems:"center"}}>
                      <input value={editLocName} onChange={e=>setEditLocName(e.target.value)}
                        ...
                        autoFocus style={{...baseInp,flex:1,fontSize:15,fontWeight:600}}/>
                    ...
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer"}} onClick={()=>{setSelLocId(loc.id);setView("location");}}>
                      <div>
                        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:5}}>
                          ...
                          <button onClick={e=>{e.stopPropagation();setEditingLocId(loc.id);setEditLocName(loc.name);}}
                            ...
                            <i className="ti ti-pencil" style={{fontSize:11}} aria-hidden="true"/>Renombrar
                          </button>
                        </div>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
                        ...
                        <i className="ti ti-chevron-right" style={{fontSize:16,color:"#d1d5db"}} aria-hidden="true"/>
                      </div>
                    </div>
                  )}
                </div>
        ...
        {view==="location"&&selLoc&&(
          <div style={{padding:"28px",maxWidth:740}}>
            <button onClick={()=>setView("locations")} style={{background:"none",border:"none",cursor:"pointer",color:"#6b7280",fontSize:13,padding:"0 0 14px",display:"flex",alignItems:"center",gap:4,fontWeight:500}}>
              <i className="ti ti-arrow-left" style={{fontSize:13}} aria-hidden="true"/> Locales
            </button>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:22}}>
              <div>
                ...
                <div style={{fontSize:13,color:"#9ca3af"}}>{locVisits.length} visita{locVisits.length!==1?"s":""} · {locOpen.length} pendiente{locOpen.length!==1?"s":""} abierto{locOpen.length!==1?"s":""}</div>
              </div>
              <button onClick={()=>{setNvLocId(selLoc.id);setView("newvisit");}} style={{display:"flex",alignItems:"center",gap:6,padding:"8px 16px",background:"#6366f1",color:"#fff",border:"none",borderRadius:8,cursor:"pointer",fontSize:13,fontWeight:600,flexShrink:0}}>
                <i className="ti ti-plus" style={{fontSize:14}} aria-hidden="true"/> Nueva visita
              </button>
            </div>
            {locOpen.length>0&&(
              <div style={{marginBottom:28}}>
            ...
            {locVisits.map(visit=>{
              ...
              return(
                <div key={visit.id} style={{background:"#fff",border:"1px solid #f3f4f6",borderRadius:12,padding:"14px 16px",marginBottom:10,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:vis.length?10:0}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:14,fontWeight:700,color:"#111827",marginBottom:bullets.length?4:0}}>{fmt(visit.date)}</div>
                      <BulletDisplay bullets={bullets}/>
                      <PhotoStrip photos={visit.photos}/>
                    </div>
                    <div style={{display:"flex",gap:8,alignItems:"center",flexShrink:0,marginLeft:12}}>
                      {vis.length>0&&<span style={{fontSize:11,fontWeight:600,color:openCount>0?"#b45309":"#166534",background:openCount>0?"#fffbeb":"#f0fdf4",padding:"2px 8px",borderRadius:8,border:`1px solid ${openCount>0?"#fcd34d":"#86efac"}`}}>{openCount>0?`${openCount} abierto${openCount!==1?"s":""}` :"✓ Todo ok"}</span>}
                  ...
                  {vis.map(item=>(
                    <div key={item.id} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 0",borderTop:"1px solid #f9fafb",flexWrap:"wrap"}}>
                      ...
                      {(item.photos||[]).length>0&&<span style={{fontSize:11,color:"#9ca3af",display:"flex",alignItems:"center",gap:3}}><i className="ti ti-photo" style={{fontSize:12}} aria-hidden="true"/>{item.photos.length}</span>}
                      {(item.comments||[]).length>0&&<span style={{fontSize:11,color:"#9ca3af",display:"flex",alignItems:"center",gap:3}}><i className="ti ti-message-circle" style={{fontSize:12}} aria-hidden="true"/>{item.comments.length}</span>}
        ...
        {view==="newvisit"&&(
          <div style={{padding:"28px",maxWidth:680}}>
            <h1 style={{fontSize:22,fontWeight:700,margin:"0 0 22px",color:"#111827"}}>Nueva Visita</h1>
            ...
            {prevOpen.length>0&&(
              <div style={{background:"#fffbeb",borderRadius:12,padding:"16px",marginBottom:20,border:"1px solid #fcd34d"}}>
                <div style={{fontSize:12,fontWeight:600,color:"#92400e",marginBottom:12,display:"flex",alignItems:"center",gap:6}}>
                  <i className="ti ti-history" style={{fontSize:14}} aria-hidden="true"/>Pendientes anteriores — marcá los resueltos hoy
                </div>
            ...
            {nvItems.length>0&&(
              <div style={{marginTop:10,marginBottom:16}}>
                {nvItems.map((item,idx)=>(
                  <div key={item.id} style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:10,padding:"10px 14px",marginBottom:6}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                      <TypeBadge type={item.type} size="sm"/>{item.area&&<AreaChip area={item.area} size="sm"/>}
                      <span style={{fontSize:13,flex:1,minWidth:120,color:"#374151"}}>{item.description}</span>
                      <button onClick={()=>setNvItems(p=>p.filter((_,i)=>i!==idx))} style={{background:"none",border:"none",cursor:"pointer",color:"#d1d5db",fontSize:14,padding:2,flexShrink:0}}><i className="ti ti-x" aria-hidden="true"/></button>
                    </div>
                    <PhotoStrip photos={item.photos}/>
                  </div>
                ))}
        ...
        {view==="editvisit"&&editVid&&(
          <div style={{padding:"28px",maxWidth:700}}>
            <button onClick={()=>setView("location")} style={{background:"none",border:"none",cursor:"pointer",color:"#6b7280",fontSize:13,padding:"0 0 14px",display:"flex",alignItems:"center",gap:4,fontWeight:500}}>
              <i className="ti ti-arrow-left" style={{fontSize:13}} aria-hidden="true"/> {editLoc?.name||"Volver"}
            </button>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:22}}>
            ...
            {editItems.filter(i=>!editDelIds.has(i.id)).map(item=>(
              <div key={item.id} style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:10,padding:"12px 14px",marginBottom:8}}>
                {editItemId===item.id?(
                  <div>
                    <InlineTypeAreaSelects type={eiType} setType={setEiType} area={eiArea} setArea={setEiArea} allFormAreas={allFormAreas} allTypeOptions={allTypeOptions} {...sharedTA}/>
                    <input value={eiDesc} onChange={e=>setEiDesc(e.target.value)} onKeyDown={e=>e.key==="Enter"&&saveEditItem(item.id)} style={{...smlInp,marginBottom:10}} placeholder="Descripción…"/>
                    <div style={{marginBottom:10}}>
                  ...
                  <div>
                    <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                      ...
                      <button onClick={()=>startEditItem(item)} style={{background:"none",border:"none",cursor:"pointer",color:"#9ca3af",fontSize:13,padding:"2px 4px"}}><i className="ti ti-pencil" style={{fontSize:13}} aria-hidden="true"/></button>
                      <button onClick={()=>deleteEditItem(item.id)} style={{background:"none",border:"none",cursor:"pointer",color:"#fca5a5",fontSize:13,padding:"2px 4px"}}><i className="ti ti-trash" style={{fontSize:13}} aria-hidden="true"/></button>
                    </div>
                    <PhotoStrip photos={item.photos}/>
                  </div>
                )}
            ...
            {showAddInEdit?(
              <div style={{marginTop:8}}><InlineAddItem {...sharedTA} onAdd={addEditItem} onCancel={()=>setShowAddInEdit(false)} showCancel/></div>
            ):(
              <button onClick={()=>setShowAddInEdit(true)} style={{width:"100%",padding:"10px",marginTop:8,border:"1.5px dashed #c7d2fe",background:"#eef2ff",color:"#4338ca",borderRadius:10,cursor:"pointer",fontSize:13,fontWeight:600,display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
                <i className="ti ti-plus" style={{fontSize:14}} aria-hidden="true"/> Agregar ítem
              </button>
            )}
          ...
          <div style={{padding:"28px",maxWidth:760}}>
            <div style={{display:"flex",gap:0,marginBottom:22,background:"#f3f4f6",borderRadius:10,padding:4,width:"fit-content"}}>
              <button onClick={()=>setShowResolved(false)} style={{padding:"7px 18px",borderRadius:7,border:"none",cursor:"pointer",fontSize:13,fontWeight:600,background:!showResolved?"#fff":"transparent",color:!showResolved?"#4338ca":"#9ca3af",boxShadow:!showResolved?"0 1px 3px rgba(0,0,0,.1)":"none"}}>
                <i className="ti ti-clock" style={{fontSize:12,marginRight:5}} aria-hidden="true"/>Abiertos ({openItems.length})
              </button>
              <button onClick={()=>setShowResolved(true)} style={{padding:"7px 18px",borderRadius:7,border:"none",cursor:"pointer",fontSize:13,fontWeight:600,background:showResolved?"#fff":"transparent",color:showResolved?"#166534":"#9ca3af",boxShadow:showResolved?"0 1px 3px rgba(0,0,0,.1)":"none"}}>
                <i className="ti ti-circle-check" style={{fontSize:12,marginRight:5}} aria-hidden="true"/>Resueltos ({resolvedItems.length})
              </button>
            </div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
              ...
              {(fArea||fType||fLoc)&&<button onClick={()=>{setFArea("");setFType("");setFLoc("");}}
                ...
                <i className="ti ti-x" style={{fontSize:12}} aria-hidden="true"/> Limpiar
              </button>}
            </div>
            {(showResolved?allResolvedAreas:allAreas).length>0&&(
              <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:16,paddingBottom:16,borderBottom:"1px solid #f3f4f6"}}>
                ...
                {(showResolved?allResolvedAreas:allAreas).map(area=>{const p=areaPalette(area);const active=fArea===area;return(
            ...
            {showResolved&&(
              <div style={{marginBottom:20}}>
                <select value={fLoc} onChange={e=>setFLoc(e.target.value)} style={{...smlInp,maxWidth:280}}>
                  <option value="">Todos los locales</option>
                  {locations.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}
                </select>
              </div>
          ...
          <div style={{padding:"28px",maxWidth:760}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:22}}>
              <div>
                ...
                <div style={{fontSize:13,color:"#6b7280"}}>Todos los puntos de análisis registrados en visitas</div>
              </div>
              <select value={fLoc} onChange={e=>setFLoc(e.target.value)} style={{...smlInp,width:"auto",minWidth:180}}>
                <option value="">Todos los locales</option>
                {locations.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}
              </select>
            </div>
              ...
              <div style={{marginBottom:24}}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                  {locations.filter(l=>items.some(i=>i.type==="analisis"&&i.locationId===l.id)).map(loc=>{
                    ...
                    return(
                      <button key={loc.id} onClick={()=>setFLoc(fLoc===loc.id?"":loc.id)}
            ...
            {allAnalysis.length===0?(
              ...
              return(
                <div key={item.id} style={{background:"#fff",border:"1px solid #e0e7ff",borderLeft:"3px solid #6366f1",borderRadius:12,padding:"14px 16px",marginBottom:8,boxShadow:"0 1px 3px rgba(99,102,241,.07)"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                    <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
                      {loc&&<span style={{fontSize:12,color:"#4338ca",fontWeight:600,display:"flex",alignItems:"center",gap:4}}><i className="ti ti-building-store" style={{fontSize:12}} aria-hidden="true"/>{loc.name}</span>}
                      {item.status==="resuelto"&&<Badge label="Resuelto" palette={PALETTE.green} icon="ti-check" size="sm"/>}
                    </div>
                  ...
                  {(item.comments||[]).length>0&&(
                    <div style={{marginTop:8,paddingTop:8,borderTop:"1px solid #f0f0ff"}}>
                      {item.comments.map(c=>(
